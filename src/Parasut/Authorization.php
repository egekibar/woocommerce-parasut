...
class Authorization extends Request
{
	 ...
	 * @param $config
	 * @throws \Plugin\Parasut\Exception
	 */
	...
	private function checkConfigData($config)
	{
		if (!isset($config["client_id"]))
			throw new \Plugin\Parasut\Exception("`client_id` parameter missing", 400);

		if (!isset($config["redirect_uri"]))
			throw new \Plugin\Parasut\Exception("`redirect_uri` parameter missing", 400);

		if (!isset($config["company_id"]))
			throw new \Plugin\Parasut\Exception("`company_id` parameter missing", 400);

		if (!isset($config["username"]))
			throw new \Plugin\Parasut\Exception("`username` parameter missing",400);

		if (!isset($config["password"]))
			throw new \Plugin\Parasut\Exception("`password` parameter missing", 400);
	}
	...
	private function accessAuthorization()
	{
		...
		{
			throw new \Plugin\Parasut\Exception($responseData["error_description"], $responseCode);
		}
	}
